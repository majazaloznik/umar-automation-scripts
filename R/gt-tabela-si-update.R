...
print("start preparing ECB data from the database")
...
path <- "\\\\192.168.38.7\\public$\\Avtomatizacija/umar-automation-scripts/data/donosnosti/Donosnosti_10Ya.xlsx"

raw <- read_xlsx(path, sheet = "Mesecno", start_row = 167, cols = 1:2,
                 col_names = FALSE)

# Find all year rows
year_rows <- which(grepl("^\\d{4}$", raw[[1]]))

# Find the last non-empty monthly row
last_data_row <- max(which(!is.na(raw[[2]]) & raw[[2]] != ""))

# Which year does this belong to?
last_year_row <- max(year_rows[year_rows < last_data_row])
last_year <- as.integer(raw[[1]][last_year_row])

# Calculate the month (position after year row)
last_month <- last_data_row - last_year_row

# remove year averages
raw <- raw[-year_rows,]
# Get last 6 rows of data
last_6_rows <- (last_data_row - 5):last_data_row - length(year_rows)
last_6_data <- raw[last_6_rows, 2]

# Calculate year/month for each of the 6 values
months <- (last_month - 5):last_month
years <- ifelse(months <= 0, last_year - 1, last_year)
months <- ifelse(months <= 0, months + 12, months)

donosnosti <- data.frame(
  period = paste(years, "m", months),
  value = as.numeric(last_6_data)) |>
  tidyr::pivot_wider(names_from = period, values_from = value) |>
  mutate(code = NA,
         Zadnja = NA) |>
  relocate(code, Zadnja)

donosnosti <- klima[0,] |>
  bind_rows(donosnosti) |>
  select(any_of(names(klima)))
...
file_configs <- list(
  list(
    path = "\\\\192.168.38.7\\data$\\GT/GT_tabele_ZA WORD_ne spreminjaj/GT_tabela_SI_angl.xlsx",
    lang = "en",
    update_text = "Last updated:"
  ),
  list(
    path = "\\\\192.168.38.7\\data$\\GT/GT_tabele_ZA WORD_ne spreminjaj/GT_tabela_SI_slo.xlsx",
    lang = "sl",
...
purrr::walk(file_configs, \(config) {
  ...
  wb$add_data(sheet = "tabela",  x = im_ex[3:8],  dims = "D14",
              na.strings = ":")
  wb$add_data(sheet = "tabela",  x =  rep(im_ex_datumi[1],4),
              colNames = FALSE,dims = "B17")
  # inflacija
  ...
  wb$add_data(sheet = "tabela",  x = place[3:8],
              dims = "D24",  na.strings = ":")
  wb$add_data(sheet = "tabela",  x = rep(place_datumi[1],8),
              colNames = FALSE, dims = "B25")
